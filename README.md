...
「修补后变化」同样在这里折算成 **0–100 的变化分**（`repairChangeScore`），前后端共用一份口径：

- **痕迹分**（权重 40）：外观痕迹等级 + 颜色匹配修正 − 外人可见；
- **尺寸分**（权重 20）：长/宽最大绝对偏移每 1 mm 扣 2 分（0 mm=100，≥50 mm=0）；**未量尺寸不计入**，权重重新归一化，绝不当成满分；
- **体感分**（权重 40）：手感 + 垂坠感等权 − 影响活动 30 分。

分档：≥85 几乎无感 / ≥70 可接受 / ≥50 明显折损 / <50 难以接受。登记修补时实时预览分值；同一破损**多轮修补**在衣物档案页按轮次列出总分、三个分项条形与相对上一已评分轮次的涨跌（`compareRepairRounds`，未填变化的轮次不切断对比链）。
...
- `tests/metrics.test.ts`：统计口径（含右删失聚合、健康分四因子、修补后变化评分与多轮涨跌）
