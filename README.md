...
「修补后变化」原本只是枚举和文字，无法量化、也看不出返工有没有变好。`packages/shared/src/repair-score.ts` 是唯一口径：

- **痕迹分**：痕迹等级 ×70% + 颜色匹配 ×30%，「外人看得出」再扣 10；
- **尺寸分**：按长/宽最大偏移量（mm）分档，未登记尺寸时该维度不参与总分（权重归一到另外两项，不白送分）；
- **体感分**：手感 ×50% + 垂坠感 ×50%，「影响活动」再扣 20；
- **总分** 按 40/30/30 加权，分优秀/良好/一般/较差四档。

档案页对同一破损的多轮修补给出逐轮分值与**与上一轮的差分**（正=变好；最大偏移是物理量，负=偏移减小），返工到底值不值一目了然；登记修补时分值随表单实时预览，导出的 Markdown / 打印档案 / CSV 用的是同一份算法。
...
- `tests/repair-score.test.ts`：修补效果分三维度折算、尺寸偏移分档、未填尺寸的权重归一、多轮差分
